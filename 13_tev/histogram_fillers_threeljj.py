...
lep3_loose_muon_mask = Lep3LooseSelectionV5
...
lep3_loose_electron_mask = Lep3LooseSelectionV5
...
def fillHistogram(cfg,t,hist,use_lhe_weight = False,is_data=False):
    ...
    for entry in range(t.GetEntries()):
        ...
        if cfg["which_selection"] == "relaxed" or cfg["which_selection"] == "relaxed_btagged":
            if not selection_threeljj.passRelaxedSelectionExceptLeptonIDs(t,cfg):
                continue
        else:
            if not selection_threeljj.passSelectionExceptLeptonIDs(t,cfg):
                continue
...
def fillHistogramsWithReweight(cfg,t,histos,mgreweight_weight_index):
    ...
    for entry in range(t.GetEntries()):
        ...
        if not selection_threeljj.passSelection(t,cfg):
            continue
